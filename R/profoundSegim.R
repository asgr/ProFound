...
profoundSegimStats=function(image=NULL, segim=NULL, mask=NULL, sky=NULL, skyRMS=NULL, 
  ...
  if(!is.na(Rapp)){
    ...
    fluxout$flux_app = temp_app$flux_app - (temp_app$N_app - Aapp)*temp_app$flux_min
  }else{
    fluxout$flux_app = NA
  }
...
profoundSegimEdge = function(segim=NULL, fill=NULL){
  ...
  temp = imager::as.cimg(segim)
  temp = as.matrix(imager::boxblur(temp, boxsize=1.1))
  segim[segim > 0L & (segim == temp)] = 0L
